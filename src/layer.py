...
# 必要ないクラス？？？
class RNN_NCE_Layer:
    def forward(self, x, prev_s, U, W, V, y=-1, sample_list=[]):
        self.mulu = mulGate.forward(U, x)
        self.mulw = mulGate.forward(W, prev_s)
        self.add = addGate.forward(self.mulw, self.mulu)
        self.s = tanh.forward(self.add)
    ...
    def backward(self, x, prev_s, U, W, V, diff_s, dmulv, y=-1, sample_list=[]):
        ...
        if sample_list != []:
            dV, dsv = mulGate.nce_backward(V, self.s, dmulv, y, sample_list)
        else:
            dV, dsv = mulGate.backward(V, self.s, dmulv)
        dV, dsv = mulGate.backward(V, self.s, dmulv)
        ds = dsv + diff_s
...
class ClassRNNLayer:
    ...
    '''
    def forward(self, x ,prev_s, U, W, V, Q, word_list, y_t):#dist=[]):
        self.mulu = mulGate.forward(U, x)
        self.mulw = mulGate.forward(W, prev_s)
        self.add = addGate.forward(self.mulw, self.mulu)
        self.s = tanh.forward(self.add)
    ...
    def backward(self, x, prev_s, U, W, V, Q, diff_s, dmulv, dmulq, word_list, y_t, class_y_t, centroids = [], word2class = [], alpha=1.0):
