...
class TwoInputModel:
    ...
    def bptt(self, x, y):
        ...
        for t in range(0, T):
            ...
            input = np.zeros(self.word_dim)
            input[x[t]] = 1
            dprev_s, dU_t, dW_t, dV_t = layers[t].backward(input, prev_s_t, self.U, self.W, self.V, self.Q, diff_s, dmulv)
            prev_s_t = layers[t].s
            ...
            for i in range(t-1, max(-1, t-self.bptt_truncate-1), -1):
                ...
                dprev_s, dU_i, dW_i, dV_i = layers[i].backward(input, prev_s_i, self.U, self.W, self.V, self.Q, dprev_s, dmulv)
                dU_t += dU_i
                dW_t += dW_i
            dV += dV_t
            dU += dU_t
        ...
        for t in range(0, T):
            ...
            input = np.zeros(self.word_dim)
            input[x[t]] = 1
            dprev_s, dU_t, dW_t, dV_t, dQ_t = layers[t].two_backward(input, prev_s_t, self.U, self.W, self.V, self.Q, diff_s, dmulv)
            prev_s_t = layers[t].s
            dmulv = np.zeros(self.word_dim)
            for i in range(t-1, max(-1, t-self.bptt_truncate-1), -1):
                input = np.zeros(self.word_dim)
                input[x[i]] = 1
                prev_s_i = np.zeros(self.hidden_dim) if i == 0 else layers[i-1].s
                dprev_s, dU_i, dW_i, dV_i, dQ_i = layers[i].two_backward(
                    input, prev_s_i, self.U, self.W, self.V, self.Q, dprev_s + dprev_prev_s, dmulv)
                dU_t += dU_i
                dW_t += dW_i
                dQ_t += dQ_i
            dV += dV_t
