...
import random
'''
Importance Samplingにより勾配の近似計算を行い、計算量を下げる->定数時間アルゴリズム？
'''
class IS_Model:
    def __init__(self, unigram, word_dim, hidden_dim=100, bptt_truncate=4):
        ...
        self.T = 100 # 損失関数の勾配の近似に用いるサンプルの個数
        '''
        self.samples = []
    ...
    def bptt(self, x, y):
        ...
        for t in range(0, T):
            ...
            dmulv = np.array([0.00000001] * self.word_dim)
            y_sample_list = []
            s = 0.0
            for i in range(self.T):
                #a = self.samples[i]
                ...
                s += np.exp(layers[t].mulv[a]) / self.q(a)
            #print("pid = %d\nt = %d"%(os.getpid(), t))
            #print(y_sample_list)
            dmulv[y[t]] = -1.0
            #print("s%d : %.4f"%(t, s))
            for v in y_sample_list:
                dmulv[v] += (np.exp(layers[t].mulv[v]) / self.q(v)) / s
            ''' 
