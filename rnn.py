...
class Model:
    def __init__(self, word_dim, hidden_dim=100, bptt_truncate=4,class_dim=0):
        ...
        if class_dim > 0:
            ...
            self.Q = np.random.uniform(-np.sqrt(1. / hidden_dim), np.sqrt(1. / hidden_dim), (1, 1))
    ...
    def bptt(self, x, y):
        ...
        for t in range(0, T):
            ...
            input = np.zeros(self.word_dim)
            input[x[t]] = 1
            dprev_s, dU_t, dW_t, dV_t, dQ_t = layers[t].backward(input, prev_s_t, self.U, self.W, self.V, diff_s, dmulv)
            prev_s_t = layers[t].s
            ...
            for i in range(t-1, max(-1, t-self.bptt_truncate-1), -1):
                ...
                dprev_s, dU_i, dW_i, dV_i, dQ_i = layers[i].backward(input, prev_s_i, self.U, self.W, self.V, dprev_s, dmulv)
                dU_t += dU_i
                dW_t += dW_i
            dV += dV_t
            dU += dU_t
